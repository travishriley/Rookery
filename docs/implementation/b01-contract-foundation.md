...
| PyYAML | 6.0.3 | [yaml/pyyaml](https://github.com/yaml/pyyaml), MIT | Development-only `safe_load` for workflow regression checks; already present in the original shared environment |
...
Executed locally in the clean environment: 19 installed-package tests and three workflow configuration tests; nine design-tool regression tests (including sixteen selected schema mutations); design validation of 230 references, 26 positive shapes, 119 negative probes, 33 local links and the unchanged prompt digest; successful wheel build/install and `pip check`. JavaScript RegExp probes independently reject final newlines/CRLF in IDs and digests. The wheel inventory contains only package modules, schema, typing marker and package metadata. The design checker and workflow explicitly exclude physical, isolation and authorization claims.

The first hosted attempt at `60006ce` failed workflow validation before any job ran: the unquoted YAML command containing `--only-binary=:all:` treated the final colon as mapping syntax. Commands now use block scalars, and three development-only YAML tests check parsing, triggers/permissions, reviewed Action pins, hosted runners and actual test commands. They guard configuration regressions, not malicious changes to both a workflow and its tests.
